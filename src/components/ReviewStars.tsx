...
export default function ReviewStars({ avg, count }: Props) {
  return (
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => {
          const fill = avg >= star ? 1 : avg >= star - 0.5 ? 0.5 : 0;
          return (
            <svg key={star} className="w-4 h-4 text-amber-400" viewBox="0 0 20 20" fill="currentColor">
              {fill === 1 ? (
      ...
      <span className="text-xs" style={{ color: "var(--text-muted)" }}>({count} รีวิว)</span>
    </div>
  );
}
