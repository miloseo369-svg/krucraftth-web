...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const dropdownLinks = [
    ...
    { href: "/wishlist", label: "คอร์สที่บันทึก", svg: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" },
    ...
    { href: "/sell", label: "ฝากขายสินค้า", svg: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" },
  ...
  return (
    <div className="min-h-screen pb-16 lg:pb-0" style={{ background: "var(--bg-primary)" }}>
      {/* Desktop nav */}
      <nav className="sticky top-0 z-50 backdrop-blur-xl border-b" style={{ background: "var(--glass-bg)", borderColor: "var(--glass-border)", backdropFilter: "blur(var(--glass-blur))" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-12 sm:h-14">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <NotificationBell />
            {/* Credits badge */}
            <Link href="/credits" className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/5 hover:bg-emerald-500/10 transition-all">
              ...
              <span className="text-xs font-medium text-emerald-400">{credits.toLocaleString()} เครดิต</span>
            </Link>
            <UserDropdown
