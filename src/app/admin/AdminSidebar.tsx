...
const mainLinks = [
  ...
  { href: "/admin/slips", label: "ตรวจสลิป", svg: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z", badge: true },
...
const toolLinks = [
  ...
  { href: "/admin/bundles", label: "แพ็คเกจ", svg: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" },
  ...
  { href: "/admin/announcements", label: "ประกาศ", svg: "M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" },
...
export default function AdminSidebar({ pendingSlips = 0 }: { pendingSlips?: number }) {
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);

  const isActive = (href: string, exact?: boolean) => exact ? pathname === href : pathname.startsWith(href);

  function renderLink(l: { href: string; label: string; svg: string; exact?: boolean; badge?: boolean }) {
    ...
    return (
      <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${active ? "bg-emerald-500/10 text-emerald-400 font-medium" : "text-[var(--text-secondary)] hover:text-white hover:bg-white/[0.04]"}`}>
        ...
        {l.badge && pendingSlips > 0 && (
          <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center">{pendingSlips}</span>
        )}
      </Link>
    );
  }
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      <div className="p-5 hidden lg:flex items-center gap-2">
        <Logo href="/admin" />
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium">Admin</span>
      </div>

      <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        <p className="text-[9px] uppercase tracking-widest font-medium text-[var(--text-muted)] px-3 pt-2 pb-1">จัดการ</p>
        {mainLinks.map(renderLink)}

        <p className="text-[9px] uppercase tracking-widest font-medium text-[var(--text-muted)] px-3 pt-4 pb-1">เครื่องมือ</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden border-t backdrop-blur-xl pb-safe" style={{ background: "var(--glass-bg)", borderColor: "var(--glass-border)" }}>
        <div className="flex items-center justify-around h-14 px-2">
          {mainLinks.map((l) => {
            const active = isActive(l.href, l.exact);
            return (
              <Link key={l.href} href={l.href} className={`relative flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg transition ${active ? "text-emerald-400" : "text-[var(--text-muted)]"}`}>
                ...
                {l.badge && pendingSlips > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-red-500 text-white text-[9px] font-bold flex items-center justify-center">{pendingSlips}</span>
                )}
              </Link>
            );
          })}
            ...
            <div className="py-2">
              {toolLinks.map((l) => (
                <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm text-white hover:bg-white/[0.04] transition">
                  <svg className="w-4 h-4 text-[var(--text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={l.svg} /></svg>
                  {l.label}
                </Link>
              ))}
